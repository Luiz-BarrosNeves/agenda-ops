from app.utils.auth import get_current_user as centralized_get_current_user print("SERVER LOADED FROM:", file) def check_role_permission(current_user, allowed_roles, action='realizar esta ação'): if current_user.role not in allowed_roles: raise HTTPException(status_code=403, detail=f'Usuário do perfil {current_user.role} não pode {action}')

def block_admin(current_user): if current_user.role == UserRole.ADMIN: raise HTTPException(status_code=403, detail='Usuário ADMIN não pode realizar esta ação')

def block_agent(current_user): if current_user.role == UserRole.AGENTE: raise HTTPException(status_code=403, detail='Usuário AGENTE não pode realizar esta ação')

def validate_agent_update_fields(update_data, apt, current_user): # AGENTE só pode alterar date e time_slot do próprio agendamento allowed_fields = {'date', 'time_slot'} for field in update_data: if field not in allowed_fields: raise HTTPException(status_code=403, detail='AGENTE só pode alterar data e horário do próprio agendamento') if apt.get('user_id') != current_user.id: raise HTTPException(status_code=403, detail='AGENTE só pode editar o próprio agendamento')

def validate_no_agent_assignment(update_data, current_user): # Comercial/Televendas não podem alterar campos de agente agent_fields = {'user_id', 'user_name', 'new_agent_id', 'new_agent_name', 'available_agent'} for field in update_data: if field in agent_fields: raise HTTPException(status_code=403, detail='Você não tem permissão para designar ou alterar agente') """ AgendaHub Backend Server ========================

Este arquivo é o ponto de entrada principal do backend. A estrutura modular está em /app/backend/app/ para organização.
...
from fastapi import FastAPI, APIRouter, HTTPException, Depends, status, UploadFile, File, Query from app.routes.users import router as users_router from fastapi.middleware.cors import CORSMiddleware from fastapi.security import HTTPBearer, HTTPAuthorizationCredentials from fastapi.responses import FileResponse, StreamingResponse from dotenv import load_dotenv from starlette.middleware.cors import CORSMiddleware from motor.motor_asyncio import AsyncIOMotorClient import os import logging import shutil import asyncio import csv import io from pathlib import Path from pydantic import BaseModel, Field, EmailStr, ConfigDict from typing import List, Optional import uuid from datetime import datetime, timezone, timedelta from zoneinfo import ZoneInfo from zoneinfo import ZoneInfo BR_TZ = ZoneInfo("America/Sao_Paulo") from zoneinfo import ZoneInfo BR_TZ = ZoneInfo("America/Sao_Paulo") import jwt from passlib.hash import bcrypt

ROOT_DIR = Path(file).parent load_dotenv(ROOT_DIR / '.env')

UPLOAD_DIR = ROOT_DIR / 'uploads' UPLOAD_DIR.mkdir(exist_ok=True)
...
app.add_middleware( CORSMiddleware, allow_origins=[ "https://agenda-ops.vercel.app", "https://agenda-ops-git-main-luiz-neves-projects.vercel.app", "http://localhost:3000", ], allow_credentials=True, allow_methods=[""], allow_headers=[""], )

from fastapi import APIRouter from app.routes.users import router as users_router from app.routes.auth import router as auth_router

api_router = APIRouter(prefix="/api") api_router.include_router(auth_router, prefix="/auth") api_router.include_router(users_router, prefix="/users")

app.include_router(api_router)

api_router = APIRouter(prefix="/api") api_router.include_router(users_router, prefix="/users") security = HTTPBearer()

from app.config import JWT_SECRET, JWT_ALGORITHM AUTO_ASSIGN_MINUTES = 5 # Tempo para atribuição automática PRESENCE_TIMEOUT_MINUTES = 3 # Tempo sem heartbeat para considerar offline auto_assign_task = None # Referência para a task de background presence_check_task = None # Referência para a task de verificação de presença

class UserRole: ADMIN = 'admin' SUPERVISOR = 'supervisor' AGENTE = 'agente' TELEVENDAS = 'televendas' COMERCIAL = 'comercial'

class UserCreate(BaseModel): email: EmailStr password: str name: str role: str avatar_url: Optional[str] = None approved: bool = False can_safeweb: bool = False can_serpro: bool = False

class UserApprove(BaseModel): approved: bool

class UserUpdateRole(BaseModel): role: str

class UserUpdatePermissions(BaseModel): can_safeweb: Optional[bool] = None can_serpro: Optional[bool] = None

class UserLogin(BaseModel): email: EmailStr password: str

class User(BaseModel): model_config = ConfigDict(extra="ignore") id: str email: str name: str role: str approved: bool avatar_url: Optional[str] = None can_safeweb: bool = False can_serpro: bool = False is_online: Optional[bool] = False last_seen: Optional[str] = None created_at: str

class AppointmentCreate(BaseModel): first_name: str last_name: str protocol_number: str additional_protocols: Optional[List[str]] = [] has_chat: bool = False chat_platform: Optional[str] = None # 'blip' ou 'chatpro' (obrigatório se has_chat=True) date: str time_slot: str notes: Optional[str] = None emission_system: Optional[str] = None # 'safeweb', 'serpro', ou None reschedule_reason: Optional[str] = None # Motivo do reagendamento (padronizado)

class AppointmentAssign(BaseModel): user_id: str

class AppointmentUpdate(BaseModel): first_name: Optional[str] = None last_name: Optional[str] = None protocol_number: Optional[str] = None additional_protocols: Optional[List[str]] = None has_chat: Optional[bool] = None chat_platform: Optional[str] = None # 'blip' ou 'chatpro' date: Optional[str] = None time_slot: Optional[str] = None appointment_type: Optional[str] = None status: Optional[str] = None notes: Optional[str] = None emission_system: Optional[str] = None reschedule_reason: Optional[str] = None # Motivo do reagendamento (padronizado)

class Appointment(BaseModel): model_config = ConfigDict(extra="ignore") id: str user_id: Optional[str] = None first_name: str last_name: str protocol_number: str additional_protocols: List[str] = [] has_chat: bool chat_platform: Optional[str] = None # 'blip' ou 'chatpro' document_urls: List[str] = [] date: str time_slot: str appointment_type: str status: str notes: Optional[str] = None emission_system: Optional[str] = None # 'safeweb', 'serpro', ou None created_by: str created_at: str updated_at: str reserved_at: str reschedule_reason: Optional[str] = None # Motivo do reagendamento (padronizado)

class NotificationCreate(BaseModel): user_id: str message: str type: str

class Notification(BaseModel): model_config = ConfigDict(extra="ignore") id: str user_id: str message: str type: str read: bool created_at: str

============== HISTORY/AUDIT MODELS ==============
class AppointmentHistory(BaseModel): model_config = ConfigDict(extra="ignore") id: str appointment_id: str action: str # created, updated, status_changed, assigned, rescheduled field_changed: Optional[str] = None old_value: Optional[str] = None new_value: Optional[str] = None changed_by: str changed_by_name: str changed_at: str

============== RECURRING APPOINTMENT MODELS ==============
class RecurringAppointmentCreate(BaseModel): first_name: str last_name: str base_protocol: str # Protocolo base para referência new_protocol: str # Novo protocolo para este reagendamento additional_protocols: Optional[List[str]] = [] has_chat: bool = False chat_platform: Optional[str] = None # 'blip' ou 'chatpro' date: str time_slot: str notes: Optional[str] = None original_appointment_id: Optional[str] = None # Referência ao agendamento original

============== APPOINTMENT TEMPLATES ==============
class AppointmentTemplateCreate(BaseModel): name: str # Nome do template (ex: "João Silva - Semanal") client_first_name: str client_last_name: str preferred_time_slot: Optional[str] = None # Horário preferido preferred_day_of_week: Optional[int] = None # 0=Segunda, 6=Domingo has_chat: bool = False chat_platform: Optional[str] = None # 'blip' ou 'chatpro' notes: Optional[str] = None tags: Optional[List[str]] = [] # Tags para organização (ex: "VIP", "Urgente")

class AppointmentTemplateUpdate(BaseModel): name: Optional[str] = None client_first_name: Optional[str] = None client_last_name: Optional[str] = None preferred_time_slot: Optional[str] = None preferred_day_of_week: Optional[int] = None has_chat: Optional[bool] = None chat_platform: Optional[str] = None # 'blip' ou 'chatpro' notes: Optional[str] = None tags: Optional[List[str]] = None

class AppointmentTemplate(BaseModel): model_config = ConfigDict(extra="ignore") id: str name: str client_first_name: str client_last_name: str preferred_time_slot: Optional[str] = None preferred_day_of_week: Optional[int] = None has_chat: bool = False chat_platform: Optional[str] = None # 'blip' ou 'chatpro' notes: Optional[str] = None tags: List[str] = [] created_by: str created_at: str updated_at: str use_count: int = 0 # Contador de vezes que foi usado last_used_at: Optional[str] = None

============== SOLICITAÇÕES DE EDIÇÃO/CANCELAMENTO ==============
class ChangeRequestType: EDIT = 'edit' CANCEL = 'cancel'

class ChangeRequestStatus: PENDING = 'pending' APPROVED = 'approved' REJECTED = 'rejected' AUTO_APPROVED = 'auto_approved' # Aprovado automaticamente por proximidade do horário

class ChangeRequestCreate(BaseModel): appointment_id: str request_type: str # 'edit' ou 'cancel' reason: Optional[str] = None # Campos para edição (opcional) new_first_name: Optional[str] = None new_last_name: Optional[str] = None new_protocol_number: Optional[str] = None new_additional_protocols: Optional[List[str]] = None new_date: Optional[str] = None new_time_slot: Optional[str] = None new_notes: Optional[str] = None

class ChangeRequestResponse(BaseModel): model_config = ConfigDict(extra="ignore") id: str appointment_id: str request_type: str status: str reason: Optional[str] = None new_first_name: Optional[str] = None new_last_name: Optional[str] = None new_protocol_number: Optional[str] = None new_additional_protocols: Optional[List[str]] = None new_date: Optional[str] = None new_time_slot: Optional[str] = None new_notes: Optional[str] = None requested_by: str requested_by_name: str created_at: str reviewed_by: Optional[str] = None reviewed_at: Optional[str] = None review_notes: Optional[str] = None

async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)): import logging logger = logging.getLogger(name) logger.warning(f"[get_current_user] credentials recebido? {credentials is not None}") token = credentials.credentials if credentials else None logger.warning(f"[get_current_user] token recebido? {token is not None}") logger.warning(f"[get_current_user] tamanho do token: {len(token) if token else 0}") if token: logger.warning(f"[get_current_user] prefixo token: {token[:10]}... (mascarado)") else: logger.warning("[get_current_user] token ausente") try: payload = jwt.decode(token, JWT_SECRET, algorithms=) user_id = payload.get('user_id') if not user_id: raise HTTPException(status_code=401, detail='Invalid token') user = await db.users.find_one({'id': user_id}, {'_id': 0}) if not user: raise HTTPException(status_code=401, detail='User not found') return User(**user) except Exception as e: logger.exception(f"[get_current_user] Exceção: {type(e).name} - {str(e)}") if isinstance(e, jwt.ExpiredSignatureError): raise HTTPException(status_code=401, detail='Token expired') if isinstance(e, jwt.InvalidTokenError): raise HTTPException(status_code=401, detail='Invalid token') raise HTTPException(status_code=401, detail='Invalid token')

@api_router.post('/auth/register', response_model=User) async def register(user_data: UserCreate): existing = await db.users.find_one({'email': user_data.email}, {'_id': 0}) if existing: raise HTTPException(status_code=400, detail='Email already registered')
...
@api_router.post('/auth/login') async def login(credentials: UserLogin): user = await db.users.find_one({'email': credentials.email}, {'_id': 0}) if not user or not bcrypt.verify(credentials.password, user['password_hash']): raise HTTPException(status_code=401, detail='Invalid credentials')
...
logger.warning(f"[jwt_login_debug] Função: login (server.py)")
logger.warning(f"[jwt_login_debug] JWT_ALGORITHM: {JWT_ALGORITHM}")
logger.warning(f"[jwt_login_debug] JWT_SECRET_ID: {secret_id}")
logger.warning(f"[jwt_login_debug] Payload: {payload}")
token = jwt.encode(payload, JWT_SECRET, algorithm=JWT_ALGORITHM)
return {
    'token': token,
    'user': User(**user)
}
@api_router.get('/auth/me', response_model=User) async def get_me(current_user: User = Depends(get_current_user)): return current_user

@api_router.get('/users', response_model=List) async def get_users( pending_approval: Optional[bool] = None, current_user: User = Depends(get_current_user) ): # Admin pode visualizar usuários, mas não pode fazer alterações if current_user.role not in ['supervisor', 'admin']: raise HTTPException(status_code=403, detail='Acesso não autorizado')
...
@api_router.put('/users/{user_id}/approve') async def approve_user(user_id: str, current_user: User = Depends(get_current_user)): if current_user.role != 'supervisor': raise HTTPException(status_code=403, detail='Only supervisors can approve users')
...
@api_router.put('/users/{user_id}/role') async def update_user_role( user_id: str, role_data: UserUpdateRole, current_user: User = Depends(get_current_user) ): if current_user.role != 'supervisor': raise HTTPException(status_code=403, detail='Only supervisors can change user roles')
...
@api_router.put('/users/{user_id}/permissions') async def update_user_permissions( user_id: str, perm_data: UserUpdatePermissions, current_user: User = Depends(get_current_user) ): """Atualizar permissões de Safeweb/Serpro do usuário""" if current_user.role != 'supervisor': raise HTTPException(status_code=403, detail='Apenas supervisores podem alterar permissões')
...
@api_router.get('/users/with-permission/{system}') async def get_users_with_permission( system: str, current_user: User = Depends(get_current_user) ): """Obter lista de usuários com permissão para um sistema específico""" if system not in ['safeweb', 'serpro']: raise HTTPException(status_code=400, detail='Sistema deve ser safeweb ou serpro')
...
return [User(**u) for u in users]
@api_router.delete('/users/{user_id}') async def delete_user(user_id: str, current_user: User = Depends(get_current_user)): if current_user.role != 'supervisor': raise HTTPException(status_code=403, detail='Only supervisors can delete users')
...
@api_router.get('/users/attendants', response_model=List) async def get_attendants(current_user: 'User' = Depends(centralized_get_current_user)): import logging logger = logging.getLogger("attendants_route") logger.warning("[attendants_route] Entrou no handler /users/attendants (server.py)") try: logger.warning(f"[attendants_route] current_user: {repr(current_user)}") logger.warning(f"[attendants_route] user_id={getattr(current_user, 'id', None)}, role={getattr(current_user, 'role', None)}") if current_user.role != UserRole.SUPERVISOR: logger.warning(f"[attendants_route] Acesso negado para usuário id={getattr(current_user, 'id', None)}, role={getattr(current_user, 'role', None)}") raise HTTPException(status_code=403, detail='Not authorized') users = await db.users.find({ 'role': {'$in': [UserRole.AGENTE]}, 'approved': True }, {'_id': 0, 'password_hash': 0}).to_list(100) logger.warning(f"[attendants_route] Retornando {len(users)} agentes para supervisor id={getattr(current_user, 'id', None)}") return [User(**u) for u in users] except Exception as e: logger.exception(f"[attendants_route] {type(e).name}: {str(e)}") raise

@api_router.get('/users/stats/team') async def get_team_stats(current_user: User = Depends(get_current_user)): if current_user.role not in [UserRole.ADMIN, UserRole.SUPERVISOR]: raise HTTPException(status_code=403, detail='Not authorized')
...
return stats
@api_router.post('/appointments', response_model=Appointment) async def create_appointment(apt_data: AppointmentCreate, current_user: User = Depends(get_current_user)): block_admin(current_user) block_agent(current_user) check_role_permission(current_user, [UserRole.TELEVENDAS, UserRole.COMERCIAL, UserRole.SUPERVISOR], 'criar agendamentos')
...
@api_router.put('/appointments/{apt_id}/assign', response_model=Appointment) async def assign_appointment( apt_id: str, assign_data: AppointmentAssign, current_user: User = Depends(get_current_user) ): if current_user.role != UserRole.SUPERVISOR: raise HTTPException(status_code=403, detail='Apenas supervisores podem atribuir agendamentos')
...
@api_router.get('/appointments', response_model=List) async def get_appointments( date: Optional[str] = None, status: Optional[str] = None, user_id: Optional[str] = None, current_user: User = Depends(get_current_user) ): query = {}
...
class RedistributionRequest(BaseModel): target_appointment_id: str # Agendamento especial (safeweb/serpro) que precisa de atribuição

class RedistributionResult(BaseModel): success: bool message: str moved_appointment_id: Optional[str] = None freed_agent_id: Optional[str] = None freed_agent_name: Optional[str] = None new_agent_id: Optional[str] = None new_agent_name: Optional[str] = None

@api_router.post('/appointments/redistribute') async def redistribute_for_special_appointment( request: RedistributionRequest, current_user: User = Depends(get_current_user) ): """ Redistribui agendamentos para liberar um agente com permissão especial. Usado quando um novo agendamento Safeweb/Serpro precisa de um agente com permissão, mas todos os agentes com permissão estão ocupados com agendamentos comuns. """ if current_user.role != UserRole.SUPERVISOR: raise HTTPException(status_code=403, detail='Apenas supervisores podem redistribuir')
...
@api_router.get('/appointments/check-redistribution/{apt_id}') async def check_redistribution_possible( apt_id: str, current_user: User = Depends(get_current_user) ): """Verificar se é possível fazer redistribuição para um agendamento especial""" apt = await db.appointments.find_one({'id': apt_id}, {'_id': 0}) if not apt: raise HTTPException(status_code=404, detail='Agendamento não encontrado')
...
@api_router.get('/appointments/paginated') async def get_appointments_paginated( page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100), date: Optional[str] = None, status: Optional[str] = None, user_id: Optional[str] = None, current_user: User = Depends(get_current_user) ): """Endpoint com paginação para agendamentos""" query = {}
...
@api_router.get('/appointments/pending', response_model=List) async def get_pending_appointments(current_user: User = Depends(get_current_user)): if current_user.role != UserRole.SUPERVISOR: raise HTTPException(status_code=403, detail='Apenas supervisores têm acesso aos agendamentos pendentes')
...
return [Appointment(**apt) for apt in appointments]
@api_router.get('/appointments/availability') async def check_availability( date: str, time_slot: str, current_user: User = Depends(get_current_user) ): agents = await db.users.find({'role': UserRole.AGENTE, 'approved': True}, {'_id': 0}).to_list(100)
...
@api_router.post('/appointments/{apt_id}/upload') async def upload_document( apt_id: str, files: List = File(...), current_user: User = Depends(get_current_user) ): apt = await db.appointments.find_one({'id': apt_id}, {'_id': 0}) if not apt: raise HTTPException(status_code=404, detail='Appointment not found')
...
@api_router.get('/appointments/{apt_id}/download/{filename}') async def download_document(apt_id: str, filename: str, current_user: User = Depends(get_current_user)): apt = await db.appointments.find_one({'id': apt_id}, {'_id': 0}) if not apt or filename not in apt.get('document_urls', []): raise HTTPException(status_code=404, detail='Document not found')
...
@api_router.delete('/appointments/{apt_id}/document/{filename}') async def delete_document(apt_id: str, filename: str, current_user: User = Depends(get_current_user)): apt = await db.appointments.find_one({'id': apt_id}, {'_id': 0}) if not apt or filename not in apt.get('document_urls', []): raise HTTPException(status_code=404, detail='Document not found')
...
@api_router.get('/appointments/available-slots') async def get_available_slots( date: str, emission_system: Optional[str] = None, current_user: User = Depends(get_current_user) ): """ Obter horários disponíveis para uma data. Se emission_system for especificado (safeweb/serpro), retorna apenas horários onde há agentes com permissão disponíveis. """ now_br = datetime.now(BR_TZ) today_br = now_br.date().isoformat() current_time_br = now_br.strftime('%H:%M') target_date = datetime.fromisoformat(date).date()
...
@api_router.get('/appointments/filtered') async def get_filtered_appointments( date_from: Optional[str] = None, date_to: Optional[str] = None, status: Optional[str] = None, agent_id: Optional[str] = None, search: Optional[str] = None, current_user: User = Depends(get_current_user) ): """Buscar agendamentos com filtros avançados""" query = {}
...
return [Appointment(**apt) for apt in appointments]
@api_router.get('/appointments/{apt_id}', response_model=Appointment) async def get_appointment(apt_id: str, current_user: User = Depends(get_current_user)): apt = await db.appointments.find_one({'id': apt_id}, {'_id': 0}) if not apt: raise HTTPException(status_code=404, detail='Appointment not found') return Appointment(**apt)

@api_router.put('/appointments/{apt_id}', response_model=Appointment) async def update_appointment( apt_id: str, apt_data: AppointmentUpdate, current_user: User = Depends(get_current_user) ):
...
@api_router.delete('/appointments/{apt_id}') async def delete_appointment(apt_id: str, current_user: User = Depends(get_current_user)): apt = await db.appointments.find_one({'id': apt_id}, {'_id': 0}) if not apt: raise HTTPException(status_code=404, detail='Appointment not found')
...
@api_router.get('/notifications', response_model=List) async def get_notifications( read: Optional[bool] = None, current_user: User = Depends(get_current_user) ): query = {'user_id': current_user.id} if read is not None: query['read'] = read

code

notifications = await db.notifications.find(query, {'_id': 0}).sort('created_at', -1).to_list(100)
return [Notification(**n) for n in notifications]
@api_router.put('/notifications/{notif_id}/read') async def mark_notification_read(notif_id: str, current_user: User = Depends(get_current_user)): await db.notifications.update_one( {'id': notif_id, 'user_id': current_user.id}, {'$set': {'read': True}} ) return {'message': 'Notification marked as read'}

@api_router.put('/notifications/read-all') async def mark_all_notifications_read(current_user: User = Depends(get_current_user)): """Marcar todas as notificações como lidas""" result = await db.notifications.update_many( {'user_id': current_user.id, 'read': False}, {'$set': {'read': True}} ) return {'message': f'{result.modified_count} notifications marked as read'}

@api_router.delete('/notifications/{notif_id}') async def delete_notification(notif_id: str, current_user: User = Depends(get_current_user)): """Excluir uma notificação""" result = await db.notifications.delete_one({ 'id': notif_id, 'user_id': current_user.id }) if result.deleted_count == 0: raise HTTPException(status_code=404, detail='Notification not found') return {'message': 'Notification deleted'}

@api_router.delete('/notifications') async def delete_all_read_notifications(current_user: User = Depends(get_current_user)): """Excluir todas as notificações lidas""" result = await db.notifications.delete_many({ 'user_id': current_user.id, 'read': True }) return {'message': f'{result.deleted_count} notifications deleted'}

@api_router.get('/stats/dashboard') async def get_dashboard_stats( date: Optional[str] = None, current_user: User = Depends(get_current_user) ): """Estatísticas para o dashboard""" if current_user.role not in [UserRole.ADMIN, UserRole.SUPERVISOR]: raise HTTPException(status_code=403, detail='Not authorized')
...
============== PRESENCE SYSTEM ==============
@api_router.post('/presence/heartbeat') async def send_heartbeat(current_user: User = Depends(get_current_user)): """Enviar heartbeat para manter status online""" now = datetime.now(timezone.utc).isoformat() await db.users.update_one( {'id': current_user.id}, {'$set': {'is_online': True, 'last_seen': now}} ) return {'status': 'online', 'timestamp': now}

@api_router.post('/presence/offline') async def go_offline(current_user: User = Depends(get_current_user)): """Marcar usuário como offline (ao fazer logout)""" await db.users.update_one( {'id': current_user.id}, {'$set': {'is_online': False, 'last_seen': datetime.now(timezone.utc).isoformat()}} ) return {'status': 'offline'}

@api_router.get('/presence/agents') async def get_agents_presence(current_user: User = Depends(get_current_user)): """Obter status de presença de todos os agentes""" if current_user.role not in [UserRole.ADMIN, UserRole.SUPERVISOR]: raise HTTPException(status_code=403, detail='Not authorized')
...
return result
============== REPORTS SYSTEM ==============
@api_router.get('/reports/daily') async def get_daily_report( date: Optional[str] = None, current_user: User = Depends(get_current_user) ): """Gerar relatório diário de atendimentos""" if current_user.role not in [UserRole.ADMIN, UserRole.SUPERVISOR]: raise HTTPException(status_code=403, detail='Not authorized')
...
@api_router.get('/reports/weekly-hours') async def get_weekly_hours( current_user: User = Depends(get_current_user) ): """Calcular saldo de horas da semana por agente""" if current_user.role not in [UserRole.ADMIN, UserRole.SUPERVISOR]: raise HTTPException(status_code=403, detail='Not authorized')
...
@api_router.get('/extra-hours') async def get_extra_hours( date: str, current_user: User = Depends(get_current_user) ): """Obter horários extras ativados para uma data""" doc = await db.extra_hours.find_one({'date': date}, {'_id': 0}) return { 'date': date, 'available_slots': EXTRA_TIME_SLOTS, 'active_slots': doc.get('slots', []) if doc else [] }

@api_router.put('/extra-hours') async def update_extra_hours( date: str, slots: List[str] = Query(default=[]), current_user: User = Depends(get_current_user) ): """Ativar/desativar horários extras para uma data (apenas supervisor)""" if current_user.role != UserRole.SUPERVISOR: raise HTTPException(status_code=403, detail='Apenas supervisores podem gerenciar horários extras')
...
@api_router.get('/slots/all') async def get_all_slots( date: str, current_user: User = Depends(get_current_user) ): """Obter todos os slots do dia (normais + extras) com status de ocupação""" # Horários normais normal_slots = [ '08:00', '08:20', '08:40', '09:00', '09:20', '09:40', '10:00', '10:20', '10:40', '11:00', '11:20', '11:40', '12:00', '12:20', '13:00', '13:20', '13:40', '14:00', '14:20', '14:40', '15:00', '15:20', '15:40', '16:00', '16:20', '16:40', '17:00', '17:20', '17:40' ]
...
async def log_appointment_history( appointment_id: str, action: str, changed_by: str, changed_by_name: str, field_changed: Optional[str] = None, old_value: Optional[str] = None, new_value: Optional[str] = None ): """Registra uma entrada no histórico do agendamento""" history_doc = { 'id': str(uuid.uuid4()), 'appointment_id': appointment_id, 'action': action, 'field_changed': field_changed, 'old_value': old_value, 'new_value': new_value, 'changed_by': changed_by, 'changed_by_name': changed_by_name, 'changed_at': datetime.now(timezone.utc).isoformat() } await db.appointment_history.insert_one(history_doc) return history_doc

@api_router.get('/appointments/{apt_id}/history') async def get_appointment_history( apt_id: str, current_user: User = Depends(get_current_user) ): """Obter histórico de alterações de um agendamento""" apt = await db.appointments.find_one({'id': apt_id}, {'_id': 0}) if not apt: raise HTTPException(status_code=404, detail='Appointment not found')
...
return history
============== CSV EXPORT ==============
@api_router.get('/reports/daily/csv') async def export_daily_report_csv( date: Optional[str] = None, current_user: User = Depends(get_current_user) ): """Exportar relatório diário em CSV""" if current_user.role not in [UserRole.ADMIN, UserRole.SUPERVISOR]: raise HTTPException(status_code=403, detail='Not authorized')
...
@api_router.get('/reports/weekly-hours/csv') async def export_weekly_hours_csv( current_user: User = Depends(get_current_user) ): """Exportar saldo de horas semanal em CSV""" if current_user.role not in [UserRole.ADMIN, UserRole.SUPERVISOR]: raise HTTPException(status_code=403, detail='Not authorized')
...
@api_router.post('/appointments/recurring', response_model=Appointment) async def create_recurring_appointment( apt_data: RecurringAppointmentCreate, current_user: User = Depends(get_current_user) ): block_admin(current_user) if current_user.role == UserRole.AGENTE: # Só pode reagendar o próprio e só pode mudar date/time_slot if apt_data.user_id != current_user.id: raise HTTPException(status_code=403, detail='AGENTE só pode reagendar o próprio agendamento') allowed_fields = {'date', 'time_slot', 'original_appointment_id', 'base_protocol', 'new_protocol'} for field in apt_data.model_dump(): if field not in allowed_fields: raise HTTPException(status_code=403, detail='AGENTE só pode alterar data e horário no reagendamento') else: check_role_permission(current_user, [UserRole.TELEVENDAS, UserRole.COMERCIAL, UserRole.SUPERVISOR], 'reagendar')
...
return Appointment(**apt_doc)
@api_router.get('/appointments/{apt_id}/recurring-info') async def get_recurring_info( apt_id: str, current_user: User = Depends(get_current_user) ): """Obter informações de recorrência de um agendamento""" apt = await db.appointments.find_one({'id': apt_id}, {'_id': 0}) if not apt: raise HTTPException(status_code=404, detail='Appointment not found')
...
============== APPOINTMENT TEMPLATES ==============
@api_router.post('/templates', response_model=AppointmentTemplate) async def create_template( template_data: AppointmentTemplateCreate, current_user: User = Depends(get_current_user) ): """Criar um novo template de agendamento""" if current_user.role not in [UserRole.TELEVENDAS, UserRole.COMERCIAL, UserRole.SUPERVISOR]: raise HTTPException(status_code=403, detail='Sem permissão para criar templates')
...
@api_router.get('/templates', response_model=List) async def get_templates( search: Optional[str] = None, tag: Optional[str] = None, current_user: User = Depends(get_current_user) ): """Listar templates de agendamento do usuário""" query = {'created_by': current_user.id}
...
@api_router.get('/templates/{template_id}', response_model=AppointmentTemplate) async def get_template( template_id: str, current_user: User = Depends(get_current_user) ): """Obter um template específico""" template = await db.appointment_templates.find_one( {'id': template_id, 'created_by': current_user.id}, {'_id': 0} ) if not template: raise HTTPException(status_code=404, detail='Template não encontrado')

code

return AppointmentTemplate(**template)
@api_router.put('/templates/{template_id}', response_model=AppointmentTemplate) async def update_template( template_id: str, template_data: AppointmentTemplateUpdate, current_user: User = Depends(get_current_user) ): """Atualizar um template""" template = await db.appointment_templates.find_one( {'id': template_id, 'created_by': current_user.id}, {'_id': 0} ) if not template: raise HTTPException(status_code=404, detail='Template não encontrado')
...
@api_router.delete('/templates/{template_id}') async def delete_template( template_id: str, current_user: User = Depends(get_current_user) ): """Excluir um template""" result = await db.appointment_templates.delete_one( {'id': template_id, 'created_by': current_user.id} ) if result.deleted_count == 0: raise HTTPException(status_code=404, detail='Template não encontrado')

code

return {'message': 'Template excluído com sucesso'}
@api_router.post('/templates/{template_id}/use') async def use_template( template_id: str, current_user: User = Depends(get_current_user) ): """Registrar uso de um template e retornar sugestão de agendamento""" template = await db.appointment_templates.find_one( {'id': template_id, 'created_by': current_user.id}, {'_id': 0} ) if not template: raise HTTPException(status_code=404, detail='Template não encontrado')
...
@api_router.post('/templates/from-appointment/{apt_id}', response_model=AppointmentTemplate) async def create_template_from_appointment( apt_id: str, template_name: str = Query(..., description="Nome do template"), current_user: User = Depends(get_current_user) ): """Criar um template a partir de um agendamento existente""" apt = await db.appointments.find_one({'id': apt_id}, {'_id': 0}) if not apt: raise HTTPException(status_code=404, detail='Agendamento não encontrado')
...
return AppointmentTemplate(**template_doc)
============== MEUS AGENDAMENTOS ==============
@api_router.get('/my-appointments') async def get_my_appointments( date: Optional[str] = None, current_user: User = Depends(get_current_user) ): """ Retorna os agendamentos do usuário atual. - Televendas/Comercial: agendamentos que criou - Agente: agendamentos atribuídos a ele - Supervisor: agendamentos que criou OU atribuídos a ele """ if current_user.role == UserRole.ADMIN: raise HTTPException(status_code=403, detail='Administradores não têm acesso a esta funcionalidade')
...
return appointments
@api_router.get('/my-appointments/stats') async def get_my_appointments_stats( current_user: User = Depends(get_current_user) ): """Retorna estatísticas dos meus agendamentos""" if current_user.role == UserRole.ADMIN: raise HTTPException(status_code=403, detail='Administradores não têm acesso a esta funcionalidade')
...
@api_router.post('/change-requests') async def create_change_request( request: ChangeRequestCreate, current_user: User = Depends(get_current_user) ): """Criar solicitação de edição ou cancelamento de agendamento""" # Buscar agendamento apt = await db.appointments.find_one({'id': request.appointment_id}, {'_id': 0}) if not apt: raise HTTPException(status_code=404, detail='Agendamento não encontrado')
...
@api_router.get('/change-requests') async def list_change_requests( status: Optional[str] = None, current_user: User = Depends(get_current_user) ): """Listar solicitações de alteração""" query = {}
...
return requests
@api_router.put('/change-requests/{request_id}/review') async def review_change_request( request_id: str, approved: bool, review_notes: Optional[str] = None, current_user: User = Depends(get_current_user) ): """Aprovar ou rejeitar uma solicitação de alteração (apenas supervisor)""" if current_user.role != UserRole.SUPERVISOR: raise HTTPException(status_code=403, detail='Apenas supervisores podem aprovar solicitações')
...
@app.on_event('shutdown') async def shutdown_db_client(): global auto_assign_task, presence_check_task if auto_assign_task: auto_assign_task.cancel() if presence_check_task: presence_check_task.cancel() client.close()

============== AUTO-ASSIGN SYSTEM ==============
async def get_next_agent_round_robin(date: str, time_slot: str, prefer_online: bool = True): """Seleciona o próximo agente usando distribuição round-robin equilibrada""" now = datetime.now(timezone.utc)
...
return agent_loads<span class="citation-group citation-pending"><span class="citation-pill">0</span></span>['agent']
async def auto_assign_pending_appointments(): """Atribui automaticamente agendamentos pendentes há mais de 5 minutos""" while True: try: await asyncio.sleep(60) # Verificar a cada 1 minuto
...
@app.on_event('startup') async def startup_event(): global auto_assign_task, presence_check_task auto_assign_task = asyncio.create_task(auto_assign_pending_appointments()) presence_check_task = asyncio.create_task(check_presence_and_redistribute()) logger.info('Auto-assign background task started') logger.info('Presence check background task started')

async def check_presence_and_redistribute(): """Verifica presença dos agentes e redistribui agendamentos de ausentes""" while True: try: await asyncio.sleep(120) # Verificar a cada 2 minutos
